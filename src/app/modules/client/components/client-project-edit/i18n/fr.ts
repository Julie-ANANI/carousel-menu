export const fr = {
  'PROJECT_EDIT': {
    'HOW': 'Comment lancer l’étude de mon projet ?',
    'ADDED': 'Ajoutée le',
    'UPDATED': 'Modifiée le',
    'STEP1': 'Définissez votre marché cible',
    'STEP2': 'Décrivez votre projet',
    'STEP3': 'Soumettez-nous votre projet',
    'EXTERNAL_DIFFUSION': 'J\'autorise UMI à diffuser mon projet auprès de sa communauté d\'ambassadeurs',
    'SUBMIT': {
      'BUTTON': 'Soumettre mon projet',
      'TITLE': 'Vous avez tout rempli ? Soumettez-nous votre projet.',
      'DESCRIPTION': {
        'LINE1': 'Après avoir reçu votre projet, nous validons le texte',
        'LINE2': 'pour lancer l’étude de façon optimale.'
      }
    },
    'TARGETING': {
      'GEOGRAPHY': 'Périmètre géographique',
      'ENTIRE_WORLD': 'Monde entier',
      'SPECIFY_COUNTRIES_TO_EXCLUDE': 'Indiquer des pays à exclure',
      'NEW_COUNTRY_TO_EXCLUDE_PLACEHOLDER': 'Ex: France'
    },
    'MARKET': {
      'LABEL': 'Secteurs d’activité',
      'PLACEHOLDER': 'Branche micro-électronique, secteur photovoltaïque, sécurité industrielle...'
    },
    'PROFESSIONALS': {
      'LABEL': 'Profils des professionnels',
      'PLACEHOLDER': 'Ingénieur R&D sur les matériaux isolants, responsable business development\nEx : John Doe chez IBM http://www.linkedin.com/in/johndoe',
      'TO_EXCLUDE': 'Indiquer des personnes à exclure'
    },
    'COMPANIES': {
      'LABEL': 'Types d’entreprises',
      'PLACEHOLDER': 'Fabricants industriels de composants, distributeurs, éditeurs de logiciels... Ex : IBM, Intel',
      'TO_EXCLUDE': 'Indiquer des entreprises à ne pas contacter'
    },
    'OTHER': {
      'LABEL': 'Compléments / Mots-clefs',
      'PLACEHOLDER': 'ANDRH, norme antibruit EN352-1, Ansys Maxwell est un logiciel concurrent…\nEx : Computational fluid dynamics, LEED Building, Moteur brushless'
    },
    'DESCRIPTION': {
      'LANGUAGE_CHOICE': 'Je décris mon projet en',
      'SET_AS_MAIN_LANG': 'Devenir langue principale',
      'MAIN_LANG': 'Langue principale',
      'TITLE': {
        'LABEL': 'Donnez un titre à votre projet',
        'PLACEHOLDER': ''
      },
      'SUMMARY': {
        'LABEL': 'Décrivez votre projet en une courte phrase',
        'PLACEHOLDER': ''
      },
      'PROBLEM': {
        'LABEL': 'Quel problème a motivé la création de votre projet ?',
        'PLACEHOLDER': ''
      },
      'SOLUTION': {
        'LABEL': 'Quelle solution votre projet apporte ?',
        'PLACEHOLDER': ''
      },
      'ADVANTAGES': {
        'LABEL': 'Quels avantages votre solution apporte-t-elle au marché ?',
        'INPUT': 'Ajouter cet avantage'
      }
    },
    'WHAT_STAGE': 'À quel stade en est votre projet ?',
    'IS_PATENTED': 'Votre projet est-t-il protégé par un brevet ?',
    'HELP': {
      'MUST_BE': 'Une bonne description doit être',
      'NO_AD': 'Non publicitaire',
      'NO_TECH': 'Non technique',
      'BRIEF': 'Concise',
      'EXAMPLES': 'Exemples de descriptions efficaces',
      'PREVIEW': 'Prévisualiser mon projet',
      'DOWNLOAD': 'Télécharger mon projet'
    },
    'MEDIA': {
      'ADD': 'Ajoutez une ou plusieurs images pour illustrer votre projet.',
      'UPLOAD': 'Télécharger des photos',
      'SLIDE': 'Vous pouvez également faire glisser les fichiers ici',
      'URL': 'Indiquer ici l\'adresse de votre vidéo'
    }
  }
};
