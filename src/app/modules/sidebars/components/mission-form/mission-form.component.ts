...
import {MailConfiguration} from '../../../../models/mail-configuration';
...
export class MissionFormComponent {
  ...
  constructor(
    ...
    this._emailService.getMailServiceConfigurations().subscribe(
      (config) => {
  ...
  public getServiceDomainLabel(index: number) {
    if (index >= 0 && index < this._missionForm.get('mailConf').value.length) {
      ...
      return `${value.service} - ${
        value.domain.length > 20
          ? value.domain.substring(0, 8) +
            '...' +
            value.domain.substring(value.domain.length - 8)
          : value.domain
      }`;
    }
  }
