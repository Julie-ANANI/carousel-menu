...
@Component({
  selector: 'app-shared-innovation-card',
  templateUrl: './shared-innovation-card.component.html',
  styleUrls: ['./shared-innovation-card.component.styl']
})
export class SharedInnovationCardComponent implements OnInit {
  ...
  getPrimary(): any {
    if(_.isEmpty(this._memPrimaryMedium)) {
      if(this._actualInnovationCard.media) {
        let primMedium = _.filter(this._actualInnovationCard.media, {'isPrimary': true});
    ...
    this._innovationService.get(innovationId).subscribe(
      innovation => {
        ...
        if (!this._innovationCards.length) {
          this._router.navigate(['/innovations']);
        }
        ...
        this._selectLangInput = this.ifIsAValidLangOption(this._langOfCard) || this.ifIsAValidLangOption(this._translateService.getBrowserLang()) || (this._filteredLangOptions[0] ? this._filteredLangOptions[0].key : 'no-more-langs');
